...
public class ComerDAO {
    ...
    public void Reservar(String fecha, ArrayList<String> dnis, String nombreEstablecimiento) {   //FU1
        ...
        try {  //MANOTE: No es muy ortodoxo este try exterior al bucle, pero no se puede poner un finally fuera de un bucle donde está en catch. No sé si funcionará esto :'(
            for (String dni : dnis) {
                try {
                    stmComer = conexion.prepareStatement("INSERT INTO asistir ('fecha', 'dni', 'nombreEstablecimiento') values (?,?,?)");  //MANOTE: No sé mucho cómo meter lo del NULL aquí, quizás así funcione al estar puntuacion de cuarto en la tabla
